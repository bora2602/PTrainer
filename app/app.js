...
const views=$$('.page'), navItems=$$('.nav-item[data-view],.tab-item[data-view]'), sidebar=$('.sidebar'), toast=$('#toast');
...
function applyTheme(theme,announce=false){const next=theme==='dark'?'dark':'light',dark=next==='dark';document.documentElement.dataset.theme=next;document.querySelector('meta[name="theme-color"]').content=dark?'#0b0707':'#fffcfc';const toggle=$('#themeToggle');if(toggle){toggle.setAttribute('aria-checked',String(dark));toggle.setAttribute('aria-label',`Switch to ${dark?'light':'dark'} theme`);$('#themeLabel').textContent=dark?'Dark theme':'Light theme';$('#themeDescription').textContent=dark?'Coral on near-black':'Coral on white';$('.theme-switch-icon').textContent=dark?'☾︎':'☀︎'}if(announce)showToast(`${dark?'Dark':'Light'} theme enabled`)}
...
navItems.forEach(item=>item.addEventListener('click',()=>switchView(item.dataset.view)));$$('[data-go]').forEach(button=>button.addEventListener('click',()=>switchView(button.dataset.go)));$('.mobile-menu').addEventListener('click',event=>{event.stopPropagation();sidebar.classList.toggle('open')});
// The drawer could only be dismissed by the button that opened it. Escape and a
// tap outside are what people already try.
document.addEventListener('keydown',event=>{if(event.key==='Escape')sidebar.classList.remove('open')});
document.addEventListener('click',event=>{if(sidebar.classList.contains('open')&&!sidebar.contains(event.target))sidebar.classList.remove('open')});$('#quickNoteButton').addEventListener('click',()=>{const id=$('#focusOpenClient').dataset.clientLink;if(!id)return switchView('clients');openClientPage(id);setTimeout(()=>$('#noteBody')?.focus(),350)});$('#helpButton').addEventListener('click',openSupportDialog);
...
$('#traineeGreeting').textContent=`Ready when you are, ${state.user.name.split(' ')[0]}`;
$('#traineeDate').textContent=new Date().toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric'});
...
if(next){
  const overdue=Boolean(next.dueDate)&&next.dueDate<data.today;
  const when=!next.dueDate?'Next workout':next.dueDate===data.today?'Today':overdue?'Missed':'Next workout';
  setTodayCard({state:overdue?'overdue':'due',when,name:next.name,
    meta:`${next.exerciseCount} exercise${next.exerciseCount===1?'':'s'}${next.dueDate&&next.dueDate!==data.today?` · ${formatDate(next.dueDate)}`:''}`,
    action:next.status==='IN_PROGRESS'?'Resume workout':'Start workout',assignmentId:next.id});
  state.activeAssignmentId=next.id;
}else{setTodayCard({state:'empty',when:'Nothing scheduled',name:'No workout waiting',meta:data.trainerName?'Your trainer’s next workout will show up here.':'Connect with a trainer to get your first workout.',action:null})}
}}

// The today card has four states and the markup says which one it is in, so
// the stylesheet can mark "missed" without a second colour and a screen reader
// gets the word rather than the styling.
function setTodayCard({state:cardState='due',when,name,meta,action=null,assignmentId=null}){
  const card=$('#todayCard'),button=$('#startTodayWorkout');
  card.classList.toggle('is-overdue',cardState==='overdue');
  card.classList.toggle('is-empty',cardState==='empty');
  $('#traineeWorkoutKicker').textContent=when;
  $('#traineeWorkoutName').textContent=name;
  $('#traineeWorkoutMeta').textContent=meta||'';
  button.hidden=!action;
  if(action)button.textContent=action;
  button.dataset.assignmentId=assignmentId||'';
}
function setTabCount(selector,count){const node=$(selector);if(!node)return;node.textContent=count>0?String(count):'';node.hidden=!(count>0)}

// One tap from opening the app to being in the workout. The old button walked
// the client to a list and left them to find the session themselves.
$('#startTodayWorkout').addEventListener('click',async event=>{
  const button=event.currentTarget,assignmentId=button.dataset.assignmentId;
  if(!assignmentId)return switchView('workouts');
  setBusy(button,true,'Opening…');
  try{await startAssignment(assignmentId)}
  catch(error){showToast(error.message||'That workout could not be opened.')}
  finally{setBusy(button,false)}
});
