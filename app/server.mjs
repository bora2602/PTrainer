...
import { nutritionTotals, targetProgress } from './nutrition-math.mjs';
...
const PUBLIC_FILES = new Set(['index.html','app.js','workouts.js','messages.js','styles.css','tokens.css','theme.css','fonts.css','nutrition-math.mjs','robots.txt']);
...
async function api(req,res,url){
  ...
  if(req.method==='GET'&&url.pathname==='/api/nutrition-entries'){
    const traineeId=await accessibleTrainee(user,url.searchParams.get('traineeId'),'view_nutrition');if(!traineeId)return json(res,403,{error:{code:'NUTRITION_FORBIDDEN',message:'Nutrition access is not allowed.'}});const selectedDate=url.searchParams.get('date');if(selectedDate&&!validDateOnly(selectedDate))return json(res,422,{error:{code:'NUTRITION_DATE_INVALID',message:'Choose a valid journal date.'}});const limit=pageLimit(url,120,500),cursor=decodeCursor(url.searchParams.get('cursor'),3);const params=[traineeId,selectedDate||null,cursor?cursor[0]:null,cursor?cursor[1]:null,cursor?cursor[2]:null,limit];const [result,target]=await Promise.all([query(`SELECT n.id,n.author_id,u.name AS author_name,to_char(n.entry_date,'YYYY-MM-DD') AS entry_date,n.entry_type,n.description,n.calories,n.protein_g::float,n.carbs_g::float,n.fat_g::float,n.water_ml,n.food_barcode,n.food_name,n.food_brand,n.food_quantity_g::float,n.data_source,n.created_at,n.updated_at FROM nutrition_entries n JOIN users u ON u.id=n.author_id WHERE n.trainee_id=$1 AND ($2::date IS NULL OR n.entry_date=$2) AND ($3::date IS NULL OR (n.entry_date, n.created_at, n.id) < ($3, $4, $5)) ORDER BY n.entry_date DESC,n.created_at DESC,n.id DESC LIMIT $6`,params),query('SELECT calories,protein_g::float,carbs_g::float,fat_g::float,water_ml,author_id,updated_at FROM nutrition_targets WHERE trainee_id=$1',[traineeId])]);const entries=result.rows.map(entry=>({...entry,can_manage:entry.author_id===user.id}));
    // Totals only mean anything for a single day; an unfiltered listing spans
    // many, so it gets none rather than a meaningless sum.
    const dayTarget=target.rows[0]||null;
    const totals=selectedDate?nutritionTotals(entries):null;
    const progress=totals&&dayTarget?Object.fromEntries(Object.entries(totals).map(([key,value])=>[key,targetProgress(value.total,dayTarget[key])])):null;
    return json(res,200,{entries,target:dayTarget,totals,progress,nextCursor:nextCursorFor(result.rows,limit,row=>[row.entry_date,row.created_at,row.id])})
  }
