...
| Load balancer and multiple replicas | Sessions now live in PostgreSQL, so sign-in survives a restart and would survive a second replica. Still process-local, and still blocking: rate-limit buckets, the idempotency replay cache, and the in-memory mirrors of users, relationships, and workout templates. The pilot must still run one app replica. |
